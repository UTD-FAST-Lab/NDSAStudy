...
class CommandLineToolRunner(AbstractCommandLineToolRunner, ABC):
    ...
    def run_from_cmd(self, cmd: List[str], job: DetectionJob, output_file: str) -> str:
        """
        ...
        ps = subprocess.run(["bash", "-c", " ".join(cmd)], stderr=subprocess.STDOUT, stdout=subprocess.PIPE, text=True)
