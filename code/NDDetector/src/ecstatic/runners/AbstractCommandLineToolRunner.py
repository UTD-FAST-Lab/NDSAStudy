...
from src.ecstatic.util.UtilClasses import DetectionJob, FinishedDetectionJob
...
class AbstractCommandLineToolRunner(ABC):
    ...
    def dict_to_config_str(config_as_dict: Dict[str, str]) -> str:
        ...
        for k in config_as_dict:
            if config_as_dict[k] == 'true':
                result+= f'--{k} '
            elif config_as_dict[k] == 'false':
                result=result;
            else:
                result+= f'--{k} {config_as_dict[k]} '
        return result.strip();

    def get_time_file(self, output_folder: str, job: DetectionJob):
        return os.path.join(output_folder,
                            '.' + os.path.basename(self.get_output(output_folder, job)) + '.time')

    def get_log_file(self, output_folder: str, job: DetectionJob):
        return os.path.join(output_folder,
    ...
    def run_job(self, job: DetectionJob, output_folder: str, num_retries: int = 1) -> FinishedDetectionJob | None:
        """
        ...
        Returns
        -------
        A FinishedDetectionJob if running the job was successful. Otherwise None.
        """
        ...
        try:
            if os.path.exists(self.get_output(output_folder, job)):
                ...
                return FinishedDetectionJob(job, execution_time, self.get_output(output_folder, job))
        except Exception:
        ...
        while num_runs < num_retries and not os.path.exists(
            ...
            try:
                ...
                with open(self.get_log_file(output_folder, job), 'w') as f:
                    f.write(log_output)
                return FinishedDetectionJob(job, total_time, result)
    ...
    @staticmethod
    def dict_hash(dictionary: Dict[str,str]) -> str:
