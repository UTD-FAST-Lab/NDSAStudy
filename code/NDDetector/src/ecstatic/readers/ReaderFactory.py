...
from src.ecstatic.readers.callgraph.OptCallGraphReader import OptCallGraphReader
from src.ecstatic.readers.callgraph.PyCGCallGraphReader import PyCGCallGraphReader
from src.ecstatic.readers.callgraph.Code2flowCallGraphReader import Code2flowCallGraphReader
from src.ecstatic.readers.callgraph.SOOTUPCallGraphReader import SOOTUPCallGraphReader
...
from src.ecstatic.readers.InferReader import InferReader
from src.ecstatic.readers.callgraph.OPALCallGraphReader import OPALCallGraphReader
    ...
    match task.lower():
        case "cg":
            match name.lower():
                ...
                case "doopc": return DOOPCallGraphReader(*args)
                case "tajs": return TAJSCallGraphReader(*args)
                case "opt": return OptCallGraphReader(*args)
                case "pycg": return PyCGCallGraphReader(*args)
                case "code2flow": return Code2flowCallGraphReader(*args)
                case "sootup": return SOOTUPCallGraphReader(*args)
                case "opal": return OPALCallGraphReader(*args)
        ...
        case "violation":
            match name.lower():
                case "infer": return InferReader(*args)
                case _: raise NotImplementedError(f"No support for task {task} on tool {name}")
